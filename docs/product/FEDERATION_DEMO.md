...
The automated proof pauses at an approval-required Admission, verifies Canonical State is unchanged, reloads the receiver browser, rediscovers the same item in the durable inbox, records a local operator approval, materializes isolated Candidate State, reruns the receiver Outcome Contract, and performs receiver-owned Promotion.
