...
Phases 0 through 11 are regression-locked on the published post-hackathon release.
The current working snapshot adds credential-safe ModelArk diagnostics, a real Codex container transaction, a Chrome-to-container acceptance gate, and additional Portable Trust policy artifacts that must preserve every earlier guarantee.
...
Typed notification intents use a candidate-owned outbox and a control-plane dispatcher that can claim an effect only after the canonical manifest advances.
The canonical proof uses the atomic local consumer, while the managed live ModelArk proof uses a real loopback HTTP receiver with receiver-enforced idempotency.
...
- Portable organizational trust policy must use a separate signature domain and authority key, and the verifier must require an evaluator-pinned authority fingerprint rather than trusting public material included by the policy itself.
...
- A Portable Evidence Packet must combine one envelope with selected optional proofs, reject any cross-receipt or altered component, and exclude producer-supplied trust policy and Authority Trust Root material.
- A Portable Decision Chain must bundle the complete signed root-to-leaf Repair lineage, reject omitted or reordered receipts, and verify exact parent digest and Canonical State continuity at every link.
...
- The interface must require regeneration after disclosure or anchor settings change and must offer one primary independently verifiable evidence-packet download while retaining expert access to the component artifacts.
...
- The independent verifier must support an optional signed Policy Authority Rotation so a pinned root can authorize one next policy key without blind re-pinning.
- The independent verifier must accept an envelope, evidence packet, or Portable Decision Chain, verify every included component and lineage link locally, and make zero API requests or uploads.
...
- The guided live server must require a fresh bounded preflight handoff that matches SHA-256 commitments to its exact configured model and provider origin before exposing live-proof mode.
- Signed live execution-profile evidence must bind the safe generated-output preflight facts to the successful Runtime profile without storing credentials, response identifiers, model output, raw model identifiers, or provider URLs.
...
- Duplicate delivery of an External Action Intent must not duplicate its accepted effect under either the atomic local consumer or the managed idempotent HTTP receiver.
...
- A signed trust policy from an unpinned authority must fail organizational authorization even when its digest and signature are mathematically valid.
...
- Immutable Discard authority exists before ordinary provider or local mutable state removal, successful provider cleanup gains an authority-bound immutable completion fact before local removal, prepare-abort authority embeds exact cleanup proof, and missing state without either proof fails recovery closed.
...
- Exactly-once delivery is claimed only for the atomic local consumer.
- The live HTTP profile claims at-least-once transport to a receiver-enforced idempotent consumer with one accepted effect identity, not distributed exactly-once delivery to arbitrary third-party providers.
