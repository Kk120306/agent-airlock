...
Accepted locally after the independent Phase 11 architecture, code, and security reviews.
...
The record commits the exact shared source, Outcome Contract, Selection Contract, bounded competitor evidence, Selection Decision, selected competitor, winner Run, and decision timestamp.
...
Every authority record for one terminal Run must commit the same terminal transaction hash, or startup treats the history as ambiguous and fails closed.
...
If startup finds a terminal Run authority while the mutable Run still appears active, it verifies the stable Run identity, source, contract, lineage, and required physical Quarantine, then replays the exact terminal transaction.
