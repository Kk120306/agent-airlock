...
| After local Quarantine removal but before final metadata                                                                  | Immutable Discard authority plus either embedded exact cleanup evidence or its provider-cleanup completion fact let startup complete without recreating mutable state.                      |
...
| A prepare-abort authority already embeds exact successful provider Discard events                                         | Recovery accepts that complete authority as cleanup proof without requiring a later cleanup fact or recreating removed Candidate State.                                                     |
| A legacy prepare-abort authority includes successful no-op Discard for an unattempted provider                            | Recovery tolerates the inert extra only after every provider known from preparation evidence has exact successful Discard coverage.                                                         |
