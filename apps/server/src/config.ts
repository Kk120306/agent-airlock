...
export function loadConfig(environment: NodeJS.ProcessEnv = process.env) {
  ...
  if (env.AIRLOCK_PROTOCOL_FIXTURE_MODE) {
    ...
    const containerProtocolFixtureProfileValid =
      loopbackHosts.has(env.HOST) &&
    ...
    const productImageProtocolFixtureProfileValid =
      env.HOST === "0.0.0.0" &&
      env.ARK_BASE_URL === "http://127.0.0.1:43991/v1" &&
      arkUrl.protocol === "http:" &&
      arkUrl.hostname === "127.0.0.1" &&
      arkUrl.pathname === "/v1" &&
      arkUrl.username === "" &&
      arkUrl.password === "" &&
      arkUrl.search === "" &&
      ...
      env.ARK_MODEL === "protocol-fixture";
    if (
      !containerProtocolFixtureProfileValid &&
      !productImageProtocolFixtureProfileValid
    ) {
      throw new Error(
        "AIRLOCK_PROTOCOL_FIXTURE_MODE requires an approved exact real-Codex protocol fixture profile",
      );
    }
  }
...
export async function writeCodexConfig(config: AppConfig): Promise<void> {
  ...
  const toml = [
    ...
    'include_only = ["PATH", "HOME", "TMPDIR", "LANG", "LC_ALL", "TERM", "NO_COLOR", "SHELL", "USER", "LOGNAME", "AIRLOCK_OUTBOX_PATH", "AIRLOCK_REPAIR_REFERENCE_PATH", "AIRLOCK_MAXIMUM_TOTAL_TOKENS"]',
    "",
