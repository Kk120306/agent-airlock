...
describe("Phase 11 Portable Trust acceptance", () => {
  ...
  it("exports, self-verifies, anchors, and ABI-encodes one promoted Run over HTTP", async () => {
    ...
    expect(verifyPortableEvidencePacket(result.packet).valid).toBe(true);
  ...
  it("chains a Repair to its quarantined parent and preserves old signatures after Discard", async () => {
    ...
    expect(child.decisionChain).not.toBeNull();
    expect(verifyPortableDecisionChain(child.decisionChain).valid).toBe(true);
    expect(child.decisionChain!.packets).toHaveLength(2);
    expect(child.decisionChain!.packets[0]!.envelope.receipt.decision).toMatchObject({
      runId: admitted.run.id,
      disposition: "quarantined",
    });
    expect(child.decisionChain!.packets[1]!.envelope.receipt.decision.runId).toBe(
      repairRunId,
    );
    ...
    expect(verifyPortableDecisionChain(repeatedChild.decisionChain).valid).toBe(true);
    expect(
      repeatedChild.decisionChain!.packets[0]!.envelope.receipt.decision.disposition,
    ).toBe("quarantined");
  });
