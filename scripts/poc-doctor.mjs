    ...
    try {
      ...
      checks.push(
        check(
          ...
          "pass",
          `A Responses API request generated non-empty assistant output after ${result.attemptCount} bounded model attempt${result.attemptCount === 1 ? "" : "s"}, ${result.requestCount ?? result.attemptCount} request${(result.requestCount ?? result.attemptCount) === 1 ? "" : "s"}, and ${result.retryDelayMs ?? 0} ms of provider-directed warm-up. No output or credential was printed.`,
        ),
      );
    } catch (error) {
      checks.push(
